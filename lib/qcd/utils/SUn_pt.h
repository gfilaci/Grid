...
Source file: ./lib/qcd/utils/SUn_pt.h
...
*************************************************************************************/
/*  END LEGAL */
#ifndef QCD_UTIL_SUN_PT_H
#define QCD_UTIL_SUN_PT_H
...
class SU {
 public:
  ...
  using iSUnMatrix = iScalar<iScalar<iScalar<iMatrix<vtype, ncolour> > > >;
  template <typename vtype>
  using iSU2Matrix = iScalar<iScalar<iScalar<iMatrix<vtype, 2> > > >;
  template <typename vtype>
  using iSUnAlgebraVector =
      iScalar<iScalar<iScalar<iVector<vtype, AdjointDimension> > > >;
