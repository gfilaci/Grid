...
int main (int argc, char ** argv)
{
...
#ifndef testadjoint
    typedef LatticeStaggeredFermion FermionType;
    typedef StaggeredFermion<QCDpt::NaiveStaggeredImplD> FermionOperator;
#else
	typedef LatticeColourMatrix FermionType;
	typedef StaggeredFermion<QCDpt::StaggeredAdjointImplNPD> FermionOperator;
#endif
  ...
  std::vector<int> seeds({12,88,46,64});
  ...
  FermionType phi        (&Grid); gaussian(pRNG,phi);
  FermionType Mphi       (&Grid);
  FermionType MphiPrime  (&Grid);
  ...
  ////////////////////////////////////
  RealD mass = 0.;
  FermionOperator Dw     (U,     Grid,RBGrid,mass);
  Dw.M   (phi,Mphi);
